...
| `SessionStore.cs` | Multi-turn sessions (orchestrator + history + feature flags; session-backed orchestrators record the conversation to the AIOrchestrator Memory at disposal — idle timeout from `AgentHarness.SuggestedConversationTimeout`; lifecycle events `SessionCreated`/`SessionRemoved` feed the OfficeManager hub). A chat request with an unknown/expired id resumes the conversation on a NEW session — seeded from the resent transcript (`session_resumed: true` in the response) — instead of failing with a 404 |
