...
| Call rings, phone hears silence, hangs up; next call gets busy (486); 200 OK retransmitted in tcpdump | **Two Kamailio bugs** (fixed 2026-08-23 in the template): (1) the 200 OK Contact of the NAT'd AgentBridge was not rewritten — ACK/BYE to the private LAN IP were unroutable, so the dialog never stabilized and the softphone held off sending RTP (the media relay never learned its address → welcome dropped). Fix: `fix_nated_contact()` in `onreply_route`. (2) `lookup("location")` ran on *in-dialog* requests too — their RURI is the Contact URI, not the agent AOR, so ACK/BYE got 404'd. Fix: route in-dialog requests (ACK/BYE/re-INVITE) with `loose_route()` + `t_relay()` only; lookup only on new INVITEs. Re-verified end-to-end: welcome + PIN + reply all arrive through the relay. |
