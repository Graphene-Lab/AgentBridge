...
| `Agent` | `"default-agent"` | Agent set used for the conversation — any preset id (see `GET /v1/models`): `default-agent`, `web-agent`, `search-agent`, `research-agent`, `document-files`, `spreadsheet-files`, `email-agent`, `office-files`, `multi-files`, `all-files` |
