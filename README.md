...
**Real office documents generated from a plain prompt** — ask for an invoice or an employment contract, attach the data, and the agent produces the finished document from the official templates, ready to use in the office. These two were generated end-to-end by the agent (material check → HTML → DOCX) in a real-user validation run:
